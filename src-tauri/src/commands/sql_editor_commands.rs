...
pub fn sql_list_connections() -> Result<Vec<SqlConnection>, String> {
    sql_editor_service::list_connections().map_err(|e| e.to_string())
}
...
pub fn sql_save_connection(
    ...
    sql_editor_service::save_connection(request).map_err(|e| e.to_string())
}

/// Xoá một kết nối theo id.
#[tauri::command]
pub fn sql_delete_connection(id: i64) -> Result<(), String> {
    sql_editor_service::delete_connection(id).map_err(|e| e.to_string())
}
