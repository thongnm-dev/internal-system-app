use crate::domain::monthly_report_import::models::{ImportBatchSummary, ImportCsvPreviewResult, ImportCsvResult};
...
#[tauri::command]
pub fn import_monthly_report_csv(app: tauri::AppHandle, path: String) -> Result<ImportCsvResult, String> {
    let app_data_dir = app.path().app_data_dir().map_err(|error| error.to_string())?;
    service::import_csv_to_database(&path, &app_data_dir).map_err(|error| error.to_string())
}
