  ...
  <section class="flex min-h-0 flex-1 flex-col gap-4 overflow-hidden">
      ...
      <div class="space-y-3">
        <div class="grid grid-cols-2 gap-3">
          <label class="block min-w-0">
            <span class="text-xs font-bold text-slate-500">Ngay doi tuong tu</span>
            <input class="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm outline-none hover:border-brand focus:border-brand focus:ring-2 focus:ring-emerald-100" type="month" :value="parseMonth(ctrl.criteria.value.target_month_from)" @change="setField('target_month_from', ($event.target as HTMLInputElement).value)" />
          </label>
          <label class="block min-w-0">
            <span class="text-xs font-bold text-slate-500">Ngay doi tuong den</span>
            <input class="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm outline-none hover:border-brand focus:border-brand focus:ring-2 focus:ring-emerald-100" type="month" :value="parseMonth(ctrl.criteria.value.target_month_to)" @change="setField('target_month_to', ($event.target as HTMLInputElement).value)" />
          </label>
        </div>

        <label class="block min-w-0">
          <span class="text-xs font-bold text-slate-500">Ten</span>
          <input class="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-brand focus:ring-2 focus:ring-emerald-100" placeholder="Ten bao cao" type="text" :value="ctrl.criteria.value.report_name" @input="setField('report_name', ($event.target as HTMLInputElement).value)" />
        </label>

        <label class="block min-w-0">
          <span class="text-xs font-bold text-slate-500">Keyword</span>
          <input class="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm outline-none focus:border-brand focus:ring-2 focus:ring-emerald-100" placeholder="Ten, ghi chu, file, nguoi import" type="text" :value="ctrl.criteria.value.keyword" @input="setField('keyword', ($event.target as HTMLInputElement).value)" @keydown.enter="ctrl.search()" />
        </label>
        ...
        <div class="flex items-center justify-end gap-2">
          ...
          <button class="flex h-10 items-center gap-2 rounded-md border border-slate-300 bg-white px-4 text-sm font-bold text-slate-600 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60" type="button" :disabled="ctrl.isSearching.value" @click="ctrl.reset()"><i class="pi pi-refresh" />Reset</button>
        </div>
      </div>
    </section>
    ...
    <section class="flex min-h-0 flex-1 flex-col overflow-hidden rounded-lg border border-stone-200 bg-white shadow-sm">
      <div class="border-b border-stone-200 px-4 py-3">
        ...
        <p class="mt-1 text-xs text-slate-500">Du lieu da duoc luu tu man hinh Import CSV.</p>
      </div>
      <DataTable class="app-data-table min-h-0" empty-message="No imported reports found." :row-class="() => 'cursor-pointer'" scrollable scroll-height="flex" :table-style="{ minWidth: '1080px' }" :value="ctrl.items.value" @row-click="(e: any) => openReport(e.data.id)">
        <Column header="SEQ" body-class="num font-bold text-slate-700" header-class="num"><template #body="{ data }">{{ `#${data.id}` }}</template></Column>
        <Column header="Ten"><template #body="{ data }"><div class="min-w-0"><strong class="block truncate text-slate-900">{{ data.report_name || '-' }}</strong><span class="mt-1 block text-xs text-slate-500">{{ formatTargetMonthRange(data) }}</span></div></template></Column>
