...
<template>
  <section class="min-h-0 flex-1 overflow-auto rounded-lg border border-stone-200 bg-panel p-4 shadow-sm">
    <div class="flex items-center justify-between gap-4">
      <button class="flex h-9 items-center gap-2 rounded-md border border-slate-300 bg-white px-3 text-sm font-bold text-slate-700 hover:bg-slate-50" type="button" @click="router.push('/projects')"><i class="pi pi-arrow-left" />Back</button>
    ...
    <p v-if="isLoading" class="mt-4 text-sm text-slate-500">Loading project information...</p>
    <p v-if="loadError" class="mt-4 rounded-lg border border-amber-200 bg-amber-50 p-3 text-sm text-amber-800">{{ loadError }}</p>

    <div class="mt-4 grid gap-4">
      <Fieldset class="rounded-lg border border-slate-200 bg-white p-4 shadow-md fieldset-nested" toggleable legend="Thong tin chung">
        <div class="grid gap-3 md:grid-cols-2">
          <label>
            <span class="text-xs font-bold text-slate-500">Project ID</span>
            <input class="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-900 outline-none" placeholder="Auto generated" readonly :value="form.projectID" />
          </label>
          <label>
            <span class="text-xs font-bold text-slate-500">Project Code</span>
            <input class="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-900 outline-none focus:border-brand focus:ring-2 focus:ring-emerald-100" placeholder="Project code" :value="form.projectCode" @input="updateForm('projectCode', ($event.target as HTMLInputElement).value)" />
          </label>
          <label class="md:col-span-2">
            <span class="text-xs font-bold text-slate-500">Project Name</span>
            <input class="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-900 outline-none focus:border-brand focus:ring-2 focus:ring-emerald-100" placeholder="Project name" :value="form.projectName" @input="updateForm('projectName', ($event.target as HTMLInputElement).value)" />
          </label>
          <Fieldset class="rounded-lg border border-stone-200 p-4 md:col-span-2 fieldset-nested" legend="Backlog" toggleable>
            <div class="grid gap-3 md:grid-cols-2">
              <label>
                <span class="text-xs font-bold text-slate-500">Backlog Project Key</span>
                <input class="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-900 outline-none focus:border-brand focus:ring-2 focus:ring-emerald-100" placeholder="BACKLOG_PROJECT_KEY" :value="form.backlogProjectKey" @input="updateForm('backlogProjectKey', ($event.target as HTMLInputElement).value)" />
              </label>
              <label>
                <span class="text-xs font-bold text-slate-500">Backlog Project ID</span>
                <input class="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-900 outline-none" :placeholder="isBacklogLookupLoading ? 'Loading...' : 'Auto fetched from Backlog'" readonly :value="form.backlogProjectID" />
              </label>
              <label class="md:col-span-2">
                <span class="text-xs font-bold text-slate-500">Backlog Project Name</span>
                <input class="mt-1 h-10 w-full rounded-md border border-slate-300 bg-white px-3 text-sm text-slate-900 outline-none" :placeholder="isBacklogLookupLoading ? 'Loading...' : 'Auto fetched from Backlog'" readonly :value="form.backlogProjectName" />
              </label>
      ...
      <Fieldset class="rounded-lg border border-stone-200 bg-white p-4 shadow-md fieldset-nested" legend="Thanh vien" toggleable>
        <div class="flex flex-wrap items-center justify-between gap-3">
          <h3 class="font-bold text-slate-800">Thanh vien</h3>
        ...
        <div class="mt-4 overflow-auto rounded-lg border border-stone-200">
          <DataTable class="app-data-table" empty-message="Chua chon thanh vien." :table-style="{ minWidth: '560px' }" :value="members">
            ...
            <Column header="Action" body-class="text-center" header-class="w-20 text-center">
              <template #body="{ data }">
                <button class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-slate-300 bg-white text-slate-600 hover:bg-slate-50" type="button" title="Remove member" @click="removeMember(data.username)"><i class="pi pi-trash" /></button>
              </template>
            </Column>
          </DataTable>
        </div>
    ...
    <div v-if="isSearchHelpOpen" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4">
      <section class="w-full max-w-2xl rounded-lg bg-white shadow-xl">
        <div class="flex items-center justify-between gap-4 border-b border-stone-200 px-4 py-3">
          ...
          <button class="flex h-9 w-9 items-center justify-center rounded-md border border-slate-300 bg-white text-slate-600 hover:bg-slate-50" type="button" title="Close" @click="isSearchHelpOpen = false"><i class="pi pi-times" /></button>
        </div>
        <div class="p-4">
          <label>
            <span class="text-xs font-bold text-slate-500">Keyword</span>
            <div class="mt-1 flex h-10 items-center rounded-md border border-slate-300 bg-white px-3 focus-within:border-brand focus-within:ring-2 focus-within:ring-emerald-100">
              <i class="pi pi-search shrink-0 text-slate-400" />
              <input class="h-full min-w-0 flex-1 border-0 px-2 text-sm text-slate-900 outline-none" placeholder="Search username or name" type="search" v-model="memberKeyword" />
            </div>
          </label>
          <div class="mt-4 max-h-[360px] overflow-auto rounded-lg border border-stone-200">
