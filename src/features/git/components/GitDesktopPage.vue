...
import type { GitBranch, GitCommit, GitFileChange, GitGraphCommit, GitRepo } from "@/_/types/git";
...
function openGraphDialog() {
  ...
  git.loadGraph(graphLimit.value);
}
...
function onGraphFile(f: GitFileChange) {
  ...
  void git.selectBrowserFile(graphSelected.value.hash, f.path);
}
...
<template>
  <section class="relative flex min-h-0 flex-1 flex-col gap-2 overflow-hidden">
    ...
    <Dialog v-model:visible="graphDialog" modal header="Visualization — đồ thị commit" :style="{ width: '1100px' }">
      <div class="flex flex-col gap-2">
        <!-- Thanh điều khiển -->
        <div class="flex items-center gap-2">
